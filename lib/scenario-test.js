...
script_test.prototype.start = function() {
  var self = this;
...
script_test.prototype.test = function(scenario) {
  ...
  if (!user) throw 'User ' + scenario.who + 'not found';
  if (!api) throw 'API ' + scenario.what + 'not found';

  var expects = utils.formatExpects(api.response['when ' + scenario.expect]);
  ...
  return new Promise(function(resolve) {
    ...
    describe(self._getDescribe(user, api), function (){
      it(self._getShould(scenario.expect), function(done){
        ...
        user.request(api, scenario.with, to)
          .then(function(res) {
            // test expects
            res.status.should.equal(expects.status);
